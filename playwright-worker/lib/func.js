...
const humanLikeInteraction = async (page) => {
  if (!page?.mouse || typeof page.evaluate !== 'function') {
    return
  }
  ...
  try {
    ...
    const safeX = (pad = 40) => clamp(randomBetween(pad, Math.max(pad + 1, viewport.width - pad)), 0, Math.max(0, viewport.width))
    const safeY = (pad = 60) => clamp(randomBetween(pad, Math.max(pad + 1, viewport.height - pad)), 0, Math.max(0, viewport.height))
    ...
    const smoothMove = async (target) => {
      const segments = randomBetween(4, 9)
      const deltaX = (target.x - currentPosition.x) / segments
      const deltaY = (target.y - currentPosition.y) / segments

      for (let i = 1; i <= segments; i++) {
        const intermediate = {
          x: Math.round(currentPosition.x + deltaX * i + randomFloat(-2, 2)),
          y: Math.round(currentPosition.y + deltaY * i + randomFloat(-2, 2)),
        }
      ...
      await randomDelay(160, 420)
    }

    await page.mouse.move(currentPosition.x, currentPosition.y, { steps: randomBetween(6, 14) })
    await randomDelay(200, 520)

    const pathIterations = randomBetween(2, 4)
    for (let i = 0; i < pathIterations; i++) {
      ...
      await smoothMove(target)

      const jitterIterations = randomBetween(1, 3)
      for (let j = 0; j < jitterIterations; j++) {
        const jitterTarget = {
          x: clamp(currentPosition.x + randomBetween(-18, 18), 0, Math.max(0, viewport.width)),
          y: clamp(currentPosition.y + randomBetween(-14, 14), 0, Math.max(0, viewport.height)),
        }

        await page.mouse.move(jitterTarget.x, jitterTarget.y, { steps: randomBetween(2, 4) })
        await randomDelay(35, 95)
        currentPosition = jitterTarget
      }
    }

    if (Math.random() > 0.55 && typeof page.mouse.click === 'function') {
      await page.mouse.down()
      await randomDelay(45, 130)
      await page.mouse.up()
      await randomDelay(180, 360)
    }

    const scrollPasses = randomBetween(1, 3)
    for (let i = 0; i < scrollPasses; i++) {
      const wheelDelta = randomBetween(220, Math.max(260, Math.floor(viewport.height * 0.65)))

      if (typeof page.mouse.wheel === 'function') {
      ...
      }, Math.round(wheelDelta * randomFloat(0.4, 0.7)))

      await randomDelay(260, 680)
    }

    if (Math.random() > 0.65 && page.keyboard) {
      await page.keyboard.press('ArrowDown', { delay: randomBetween(40, 160) }).catch(() => {})
      await randomDelay(220, 520)
    }
  } catch (error) {
    if (debug) {
      logger.warn('humanLikeInteraction failed', { error })
    }
  }
}

const simulatePreNavigation = async (page, targetUrl) => {
  if (!page) return

  try {
    await page.bringToFront().catch(() => {})
    await sleep(randomIntegerBetween(120, 260))

    const probableFocusPoint = {
      x: randomIntegerBetween(140, 320),
      y: randomIntegerBetween(40, 90),
    }

    await page.mouse.move(probableFocusPoint.x, probableFocusPoint.y, { steps: randomIntegerBetween(4, 8) })
    await sleep(randomIntegerBetween(80, 160))
    await page.mouse.down({ button: 'left' })
    await page.mouse.up({ button: 'left' })

    if (targetUrl) {
      const tokens = targetUrl.replace(/^https?:\/\//, '').split(/[/?#]/)
      const typedSegments = tokens.slice(0, 2).join('/')
      await page.keyboard.type(typedSegments, { delay: randomIntegerBetween(35, 90) })
      await sleep(randomIntegerBetween(150, 240))
      await page.keyboard.press('Backspace', { delay: randomIntegerBetween(40, 70) })
      await page.keyboard.type('/', { delay: randomIntegerBetween(40, 70) })
      await sleep(randomIntegerBetween(120, 200))
      await page.keyboard.down('Shift')
      await page.keyboard.press('KeyA', { delay: randomIntegerBetween(40, 70) })
      await page.keyboard.up('Shift')
    }

    await sleep(randomIntegerBetween(150, 220))
  } catch (error) {
    if (debug) {
      logger.warn('simulatePreNavigation failed', { error })
    }
  }
}
...
module.exports = {
  ...
  humanLikeInteraction: async (page) => {
  ...
  simulatePreNavigation: async (page, url) => {
    await simulatePreNavigation(page, url)
  },
