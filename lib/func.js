...
const buildDefaultSafariHeaders = (languages = DEFAULT_LANGUAGES) => ({
  Accept: DEFAULT_ACCEPT_HEADER,
  'Accept-Language': buildAcceptLanguageHeader(languages),
...
const upsertHeader = (headers, name, value) => {
  if (!headers || !name) {
    return
  }
...
const mergeCustomHeaders = (defaultHeaders = {}, overrides = []) => {
  ...
  overrides.forEach((entry) => {
    const name = (entry?.header || '').trim()

    if (!name) {
      return
    }

    upsertHeader(headers, name, entry?.value)
  })
...
const buildHeaderConfig = (job) => {
  ...
  const languages = profile?.languages || DEFAULT_LANGUAGES
  const headers = mergeCustomHeaders(buildDefaultSafariHeaders(languages), job.args?.headers)
  ...
  ensureHeader(headers, 'Accept-Language', buildAcceptLanguageHeader(languages))
