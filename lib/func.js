...
module.exports = {
  ...
  addJsCode: async (page, job) => {
    ...
    try {
      ...
      logger.error('Failed to evaluate page', e)
    }
  ...
  addCssCode: async (page, job) => {
    ...
    try {
      ...
      logger.error('Failed to add style tag', e)
    }
  ...
  addFixtures: async (page, job) => {
    ...
    await page.evaluate((_fixtures) => {
      ...
      function diffyImageFixture (el, selector, logger) {
        return new Promise((resolve, reject) => {
          try {
            ...
            // logger.info('[DiffyImageFixture] Processing element: ', selector, src, h, w)
            ...
            if (src && w && h) {
              ...
              el.addEventListener('error', (e) => {
                logger.error('Failed to diffy image fixture', e)
                reject(e);
              });
              ...
              // logger.info('[DiffyImageFixture] New src: ', selector, src, el.src)
              ...
              logger.warn('Can\'t add diffy image fixture', selector, src, h, w)
              return resolve()
            }
          } catch (e) {
            logger.error('Failed to diffy image fixture', e)
      ...
      function diffyBackgroundImageFixture (el, logger) {
        return new Promise((resolve) => {
          try {
            ...
            // logger.info('Processing element: ', selector)
            ...
            getImageInfo(backgroundImage, logger)
              .then((imageInfo) => {
      ...
      function getImageInfo (url, logger) {
        return new Promise((resolve, reject) => {
          const img = new Image();
      ...
      function diffyTextFixture (el, content, logger) {
        return new Promise((resolve) => {
          try {
            el.innerHTML = content
          } catch (e) {
            logger.error('Failed to diffy text fixture', e)
          }
      ...
      for (let fixture of _fixtures) {
        ...
        for (let i = 0; i < elementKeys.length; ++i) {
          if (type === 'image') {
            fixturePromises.push(diffyImageFixture(element[elementKeys[i]], selector, logger))
          } else if (type === 'background image') {
            fixturePromises.push(diffyBackgroundImageFixture(element[elementKeys[i]], logger))
          } else {
            fixturePromises.push(diffyTextFixture(element[elementKeys[i]], content, logger))
          }
        }
      }
  ...
  auth: async (page, job) => {
    ...
    try {
      ...
      logger.warn("Navigation after login failed, retrying...");
