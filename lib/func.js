...
module.exports = {
  ...
  auth: async (page, job) => {
    ...
    const needIncrease = job?.args?.need_increase ?? job?.need_increase;
    const timeouts = {
      goto: resolveTimeoutWithNeedIncrease(needIncrease, 30000),
      awaitResponse: resolveTimeoutWithNeedIncrease(needIncrease, 60000),
      postSubmitNavigation: resolveTimeoutWithNeedIncrease(needIncrease, 120000),
    };
    ...
    await page.goto(url, { waitUntil: ['domcontentloaded', 'networkidle2'], timeout: timeouts.goto })
    await awaitResponse(page, timeouts.awaitResponse)
    ...
    try {
      ...
      await page.waitForNavigation({ waitUntil: ['domcontentloaded', 'networkidle2'], timeout: timeouts.postSubmitNavigation });
