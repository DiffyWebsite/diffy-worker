...
const updatePageViewport = async (page, job, maxPageHeight = null) => {
  let scrollHeight = await Promise.race([
    page.evaluate(`(async () => {
        return document.documentElement.scrollHeight;
    })()`),
    new Promise((_, rej) => setTimeout(() => rej(new Error('scrollHeight timeout')), 4000)),
  ]).catch(() => 1000);
  ...
  await Promise.race([
    page.setViewport({ width: Number.parseInt(job.breakpoint), height: Number.parseInt(scrollHeight) }),
    new Promise((_, rej) => setTimeout(() => rej(new Error('setViewport timeout')), 4000)),
  ]).catch(() => {});
...
module.exports = {
  ...
  addJsCode: async (page, job) => {
    ...
    try {
      await Promise.race([
        page.evaluate(job.args.js_code),
        new Promise((_, rej) =>
            setTimeout(() => rej(new Error('evaluate js_code timeout')), 5000)
        ),
      ]);
    } catch (e) {
  ...
  addCssCode: async (page, job) => {
    ...
    try {
      await Promise.race([
        page.addStyleTag({ content: job.args.css_code }),
        new Promise((_, rej) => setTimeout(() => rej(logger.warn('addStyleTag timeout')), 4000)),
      ]);
    } catch (e) {
