...
module.exports = {
  ...
  addJsCode: async (page, job) => {
    ...
    try {
      ...
      logger.error(job.id + ':' + job.breakpoint + ':' + job.url, 'Failed to evaluate page', e)
    }
  ...
  addCssCode: async (page, job) => {
    ...
    try {
      ...
      logger.error(job.id + ':' + job.breakpoint + ':' + job.url, 'Failed to add style tag', e)
    }
  ...
  addFixtures: async (page, job) => {
    ...
    logger.info(job.id + ':' + job.breakpoint + ':' + job.url, 'Diffy fixtures were added.')
  ...
  auth: async (page, job) => {
    ...
    logger.info(job.id + ':' + job.breakpoint + ':' + job.url, `Navigating to ${url}`);
    ...
    if (checkArgs(job, 'before_login_css')) {
      logger.info(job.id + ':' + job.breakpoint + ':' + job.url, "Clicking before login element.");
    ...
    if (job.args.usernameSelector) {
      ...
      logger.info(job.id + ':' + job.breakpoint + ':' + job.url, "Typing username.");
    ...
    logger.info(job.id + ':' + job.breakpoint + ':' + job.url, "Typing password.");
    ...
    logger.info(job.id + ':' + job.breakpoint + ':' + job.url, "Clicking submit button.");

    try {
      await page.focus(job.args.submitSelector);
      await page.click(job.args.submitSelector);
      logger.info(job.id + ':' + job.breakpoint + ':' + job.url, "Submit button clicked successfully.");
    } catch (error) {
      logger.error(job.id + ':' + job.breakpoint + ':' + job.url, "Failed to click submit button.", error);
      return Promise.resolve();
    }

    try {
      const preNavUrl = await page.url();
      logger.info(job.id + ':' + job.breakpoint + ':' + job.url, `Current URL before navigation: ${preNavUrl}`);

      const preNavTitle = await page.title();
      logger.info(job.id + ':' + job.breakpoint + ':' + job.url, `Page title before navigation: ${preNavTitle}`);

      await page.waitForSelector('#toolbar-administration', { timeout: 120000 });

      const postNavUrl = await page.url();
      logger.info(job.id + ':' + job.breakpoint + ':' + job.url, `New URL after navigation: ${postNavUrl}`);
      const postNavTitle = await page.title();
      logger.info(job.id + ':' + job.breakpoint + ':' + job.url, `Page title after navigation: ${postNavTitle}`);

      logger.info(job.id + ':' + job.breakpoint + ':' + job.url, "Navigation after login successful.");
    } catch (error) {
      logger.error(job.id + ':' + job.breakpoint + ':' + job.url, 'Navigation after login failed', error);

      const errorUrl = await page.url();
      const errorTitle = await page.title();
      logger.error(job.id + ':' + job.breakpoint + ':' + job.url, `URL at failure: ${errorUrl}`);
      logger.error(job.id + ':' + job.breakpoint + ':' + job.url, `Title at failure: ${errorTitle}`);

      await page.screenshot({ path: `navigation-failure-${job.id}.png`, fullPage: true });

      return Promise.resolve();
    }

    logger.info(job.id + ':' + job.breakpoint + ':' + job.url, "Authentication process completed.");
    return page.cookies()
  },
  ...
  getImageSize: async (file) => {
    try {
        ...
        logger.error(url, 'Failed to get file metadata', err);
