...
module.exports = {
  ...
  addJsCode: async (page, job) => {
    ...
    try {
      ...
      logger.error(job.args.breakpoint + ':' + job.args.url, 'Failed to evaluate page', e)
    }
  ...
  addCssCode: async (page, job) => {
    ...
    try {
      ...
      logger.error(job.args.breakpoint + ':' + job.args.url, 'Failed to add style tag', e)
    }
  ...
  addFixtures: async (page, job) => {
    ...
    logger.info(job.args.breakpoint + ':' + job.args.url, 'Diffy fixtures were added.')
  ...
  auth: async (page, job) => {
    ...
    logger.info(job.args.breakpoint + ':' + url, `Navigating to ${url}`);
    ...
    if (checkArgs(job, 'before_login_css')) {
      logger.info(job.args.breakpoint + ':' + url, "Clicking before login element.");
    ...
    if (job.args.usernameSelector) {
      ...
      logger.info(job.args.breakpoint + ':' + url, "Typing username.");
    ...
    logger.info(job.args.breakpoint + ':' + url, "Typing password.");
    ...
    logger.info(job.args.breakpoint + ':' + url, "Clicking submit button.");
    await page.focus(job.args.submitSelector)
    await page.click(job.args.submitSelector);

    try {
      await page.waitForNavigation({ waitUntil: 'networkidle2', timeout: 120000 })
      logger.info(job.args.breakpoint + ':' + url, "Navigation after login successful.");
    } catch (error) {
      logger.error(job.args.breakpoint + ':' + url, 'Navigation after login failed', error)
      // Retry logic or handle the failure as needed
      return Promise.resolve();
    }

    logger.info(job.args.breakpoint + ':' + url, "Authentication process completed.");
    return page.cookies()
  },
