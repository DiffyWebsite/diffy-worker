  ...
  fixViewportRelativeHeights: async (page, cleanup = false) => {
    try {
      return await page.evaluate((shouldCleanup) => {
        ...
        const entries = window.__diffyRecordedHeights.slice().reverse()
        const fixedEntries = []
        ...
        for (const entry of fixedEntries) {
          const savedHeight = entry.node.style.height
          const savedMaxHeight = entry.node.style.maxHeight
          const savedMinHeight = entry.node.style.minHeight

          entry.node.style.height = ''
          entry.node.style.maxHeight = ''
          entry.node.style.minHeight = ''

          const naturalHeight = entry.node.offsetHeight

          if (naturalHeight > entry.height) {
            // Element grows without the fix — restore it
            entry.node.style.height = savedHeight
            entry.node.style.maxHeight = savedMaxHeight
            entry.node.style.minHeight = savedMinHeight
          } else {
            // Element naturally shrinks or stays the same — let it flow
            fixedCount--
          }
        }
