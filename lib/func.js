...
module.exports = {
  ...
  addJsCode: async (page, job) => {
    ...
    try {
      ...
      logger.error(job.args.url, 'Failed to evaluate page', e)
    }
  ...
  addCssCode: async (page, job) => {
    ...
    try {
      ...
      logger.error(job.args.url, 'Failed to add style tag', e)
    }
  ...
  addFixtures: async (page, job) => {
    ...
    logger.info(job.args.url, 'Diffy fixtures were added.')
    logger.error(job.args.url, 'Diffy fixtures were added.')
    logger.warn(job.args.url, 'Diffy fixtures were added.')
  ...
  auth: async (page, job) => {
    ...
    if (checkArgs(job, 'before_login_css')) {
      logger.info(url, "Clicking before login element.");
    ...
    if (job.args.usernameSelector) {
      ...
      logger.info(url, "Typing username.");
    ...
    logger.info(url, "Typing password.");
    ...
    logger.info(url, "Clicking submit button.");
    await page.focus(job.args.submitSelector)
    await page.click(job.args.submitSelector);

    try {
      await page.waitForNavigation({ waitUntil: 'networkidle2', timeout: 120000 })
      logger.info(url, "Navigation after login successful.");
    } catch (error) {
      logger.error(url, 'Navigation after login failed', error)
      // Retry logic or handle the failure as needed
      return Promise.resolve();
    }

    logger.info(url, "Authentication process completed.");
    return page.cookies()
  },
