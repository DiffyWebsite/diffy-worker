...
module.exports = {
  ...
  addJsCode: async (page, job) => {
    ...
    try {
      ...
      logger.error(job.breakpoint + ':' + job.url, 'Failed to evaluate page', e)
    }
  ...
  addCssCode: async (page, job) => {
    ...
    try {
      ...
      logger.error(job.breakpoint + ':' + job.url, 'Failed to add style tag', e)
    }
  ...
  addFixtures: async (page, job) => {
    ...
    logger.info(job.breakpoint + ':' + job.url, 'Diffy fixtures were added.')
  ...
  auth: async (page, job) => {
    ...
    logger.info(job.breakpoint + ':' + job.url, `Navigating to ${url}`);
    ...
    if (checkArgs(job, 'before_login_css')) {
      logger.info(job.breakpoint + ':' + job.url, "Clicking before login element.");
    ...
    if (job.args.usernameSelector) {
      ...
      logger.info(job.breakpoint + ':' + job.url, "Typing username.");
    ...
    logger.info(job.breakpoint + ':' + job.url, "Typing password.");
    ...
    logger.info(job.breakpoint + ':' + job.url, "Clicking submit button.");
    await page.focus(job.args.submitSelector)
    await page.click(job.args.submitSelector);

    try {
      await page.waitForNavigation({ waitUntil: 'networkidle2', timeout: 120000 })
      logger.info(job.breakpoint + ':' + job.url, "Navigation after login successful.");
    } catch (error) {
      logger.error(job.breakpoint + ':' + job.url, 'Navigation after login failed', error)
      // Retry logic or handle the failure as needed
      return Promise.resolve();
    }

    logger.info(job.breakpoint + ':' + job.url, "Authentication process completed.");
    return page.cookies()
  },
