...
module.exports = {
  ...
  auth: async (page, job) => {
    ...
    if (checkArgs(job, 'before_login_css')) {
      ...
      try {
        ...
        logger.warn('Before login css error', { error })
      }
    ...
    try {
      ...
      // Add a 10-second delay to allow any asynchronous operations to complete before retrieving cookies
  ...
  setHeaders: async (page, job) => {
    ...
    if (userAgentString.length) {
      logger.info('User-Agent: "' + userAgentString + '"');
