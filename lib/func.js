...
// Headers that are applied at the browser-context level (Playwright `userAgent`) and
// therefore must stay identical on every request, same-origin or not.
const CONTEXT_LEVEL_HEADERS = new Set(['user-agent'])
...
/**
 * Origin ("scheme://host[:port]") of an http(s) URL, lowercased. Empty for anything else.
 */
...
/**
 * Origins that belong to the job itself: the page under test, its base URL and the
 * login page. Custom headers are only ever sent to these.
 */
const buildTargetOrigins = (job) => {
  const origins = []
  const candidates = [job?.url, job?.base_url, job?.args?.url]
...
const isTargetOriginRequest = (headerState, requestUrl) => {
  const origins = headerState?.targetOrigins
...
const buildHeaderState = (job) => {
  ...
  const headers = {}
  // Subset of `headers` that must be scoped to the job's own origins. Sending these on
  // cross-origin requests turns them into preflighted CORS requests, and third-party hosts
  // (fonts.gstatic.com and friends) reject the preflight, so the resource fails outright.
  const customHeaders = {}

  if (job?.args?.headers) {
    job.args.headers.forEach(element => {
      if (element.header && element.header.trim().length) {
        const headerName = element.header.trim()
        headers[headerName] = element.value

        if (!CONTEXT_LEVEL_HEADERS.has(headerName.toLowerCase())) {
          customHeaders[headerName] = element.value
        }
      }
    })
  }

  const targetOrigins = buildTargetOrigins(job)
  const customHeaderNames = Object.keys(customHeaders)

  if (customHeaderNames.length) {
    if (targetOrigins.length) {
      logger.debug('Custom headers scoped to job origins', {
        headers: customHeaderNames,
        targetOrigins
      })
    } else {
      logger.warn('Custom headers configured but no target origin could be resolved; headers will not be sent', {
        headers: customHeaderNames
      })
    }
  }
...
module.exports = {
  ...
  /**
   * Inject the project custom CSS on every navigation.
   *
   * auth() navigates to the login page itself, so a style tag added beforehand is
   * discarded. An init script survives that navigation and lands before the page
   * scripts run, which is what it takes to hide an overlay that would otherwise
   * cover the login form.
   */
  addCssCodeOnNavigation: async (page, job) => {
    if (!checkArgs(job, 'css_code', true)) {
      return Promise.resolve()
    }

    try {
      await page.addInitScript((cssCode) => {
        const injectStyle = () => {
      ...
      logger.error('Failed to add init style script', { error: e, css_code: job.args.css_code })
    }
    ...
    try {
      return await page.evaluate((_elements) => {
        ...
        const maskElement = (element) => {
          if (masked.has(element) || !isVisible(element)) {
            return
          }
        ...
        const selectors = (Array.isArray(_elements) ? _elements : [])
          .filter((selector) => typeof selector === 'string' && selector.trim().length)
          .map((selector) => selector.trim())
        ...
        setInterval(() => {
          try {
            ...
            // an invalid selector must not kill the re-scan loop
          }
        }, 500)
      }, job.args.elements)
    } catch (e) {
  ...
  auth: async (page, job) => {
    ...
    try {
      ...
      logger.warn('Submit click intercepted, falling back to DOM click.', { error: describeError(error), submitSelector: job.args.submitSelector })
      await page.$eval(job.args.submitSelector, (el) => el.click())
    }
  ...
  /**
   * Resolve the header state for a job.
   *
   * Custom headers are deliberately NOT installed with `page.setExtraHTTPHeaders`: that
   * applies them to every request, including cross-origin ones. A non-safelisted header on a
   * cross-origin request makes the browser preflight it, and third-party hosts such as
   * fonts.gstatic.com do not list those headers in Access-Control-Allow-Headers, so the
   * request dies with ERR_FAILED and the web fonts never load. Use the helpers below to
   * attach them per request, target origin only.
   */
  buildHeaderState: (job) => {
    const state = buildHeaderState(job)
  ...
  /**
   * True when the request goes to one of the job's own origins, i.e. the only place custom
   * headers may be sent.
   */
  shouldApplyCustomHeaders: (headerState, requestUrl) => {
    if (!headerState?.customHeaderNames?.length) {
      return false
    }
    return isTargetOriginRequest(headerState, requestUrl)
  },

  /**
   * Lowercased names of custom headers present on a request that must not carry them.
   * Belt and braces: nothing sets them globally any more, but anything that slips through
   * (redirect replays, page-initiated fetches copying headers) is removed before the request
   * leaves the browser.
   */
  findLeakedCustomHeaders: (headerState, requestHeaders) => {
    if (!headerState?.customHeaderNames?.length || !requestHeaders) {
      return []
    }

    return headerState.customHeaderNames
      .map((name) => name.toLowerCase())
      .filter((name) => Object.hasOwn(requestHeaders, name))
  },

  /**
   * Copy the job's custom headers onto a request header map, replacing any casing variant
   * already present so the request cannot end up with the header twice.
   */
  applyCustomHeaders: (headerState, headers) => {
    if (!headerState?.customHeaderNames?.length || !headers) {
      return headers
    }

    headerState.customHeaderNames.forEach((name) => {
      const lowerName = name.toLowerCase()
      if (lowerName !== name && Object.hasOwn(headers, lowerName)) {
        delete headers[lowerName]
      }
      headers[name] = String(headerState.customHeaders[name] ?? '')
    })
