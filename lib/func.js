...
module.exports = {
  ...
  detectStretchedElements: async (page) => {
    try {
      return await page.evaluate(() => {
        const results = { images: [], blocks: [] }
        const viewportWidth = window.innerWidth
        const viewportHeight = window.innerHeight
        const IMG_RATIO_THRESHOLD = 0.05
        const BLOCK_HEIGHT_RATIO = 3
        const VIEWPORT_MATCH_THRESHOLD = 0.05
        ...
        for (const img of document.querySelectorAll('img')) {
          ...
          if (ratioDiff > IMG_RATIO_THRESHOLD) {
            const src = img.src || ''
            results.images.push({
        ...
        for (const el of document.querySelectorAll('div,section,article,header,footer,main,aside,nav')) {
          const rect = el.getBoundingClientRect()
          if (rect.width === 0 || rect.height === 0) continue
          const heightToWidthRatio = rect.height / viewportWidth
          const matchesViewportHeight = Math.abs(rect.height - viewportHeight) / viewportHeight < VIEWPORT_MATCH_THRESHOLD

          if (heightToWidthRatio > BLOCK_HEIGHT_RATIO || matchesViewportHeight) {
            results.blocks.push({
              tag: el.tagName.toLowerCase(),
              id: el.id || null,
              className: (el.className && typeof el.className === 'string')
                ? el.className.trim().substring(0, 100) : null,
              renderedWidth: Math.round(rect.width),
              renderedHeight: Math.round(rect.height),
              heightToViewportWidthRatio: Math.round(heightToWidthRatio * 100) / 100,
              matchesViewportHeight,
            })
          }
        }
