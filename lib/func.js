...
module.exports = {
  ...
  hideBanners: async (page, job) => {
    ...
    const isGetPositionExistant = await page.evaluate(() => {
      return !!window.getPosition;
    });

    if (!isGetPositionExistant) {
      await page.exposeFunction('getPosition', getPosition);
    }

    return page.evaluate((_elements) => {

      const vrtPaintOver = async (element) => {
        const rectObject = await window.getPosition(element)
        if (!rectObject) {
          return
        }
  ...
  cropElement: async (page, job) => {
    ...
    return page.evaluate(async (_selector) => {
      ...
      window.scrollTo(0, 0)
      const position = await window.getPosition(document.querySelector(_selector))
      return position;
