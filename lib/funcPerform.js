...
const { ChromiumBrowser } = require('./chromiumBrowser');
...
module.exports = {
  ...
  perform: async (browser, job, jobItem) => {
    ...
    try {
      ...
      const chromiumBrowserInstance = new ChromiumBrowser(true, false);

      const useProxy = jobItem.proxy;

      if (useProxy) {
        page = await chromiumBrowserInstance.newPage(browser, process.env.PROXY);
      } else {
