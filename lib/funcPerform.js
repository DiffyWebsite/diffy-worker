...
module.exports = {
  ...
  perform: async (browser, job, jobItem) => {
    ...
    try {
      ...
      logger.info(jobItem.url, 'setDefaultNavigationTimeout done', jobItem.breakpoint + ' ' + jobItem.url)
      page.on('console', msg => {
        let consoleMes
        try {
          consoleMes = {
      ...
      logger.info(jobItem.url, 'setHeaders done')
      ...
      logger.info(jobItem.url, 'addCookies done')
      ...
      logger.info(jobItem.url, 'auth done')
      if (authCookies) {
        logger.info(jobItem.url, 'authCookies', authCookies)
      ...
      try {
        ...
        logger.info(jobItem.url, 'page was not loaded by networkidle2')
        await page.goto(url, { timeout: 60000, waitUntil: ['domcontentloaded'] })
      }
      logger.info(jobItem.url, 'page loaded done')
      ...
      if (Object.hasOwn(jobItem.args, 'disable_css_animation') && jobItem.args.disable_css_animation) {
        logger.info(jobItem.url, 'disable css animation')
      ...
      logger.info(jobItem.url, 'page.goto done')
      ...
      logger.info(jobItem.url, 'addCssCode done')
      ...
      logger.info(jobItem.url, 'autoScroll done')
      ...
      logger.info(jobItem.url, 'updatePageViewport done', { page_height })
      ...
      logger.info(jobItem.url, 'addJsCode done')

      logger.info(jobItem.url, 'delayBeforeScreenshot done')
      ...
      logger.info(jobItem.url, 'cutElements done')
      await func.hideBanners(page, jobItem)
      logger.info(jobItem.url, 'hideBanners done')

      await func.addFixtures(page, jobItem)
      logger.info(jobItem.url, 'addFixtures done')
      ...
      logger.info(jobItem.url, 'start screenshot')
      ...
      logger.info(jobItem.url, 'screenshot done')
      const pageHtml = await func.getPageHtml(page)
      logger.info(jobItem.url, 'pageHtml done')
      if (is_crop) {
      ...
      await page.close()
      logger.info(jobItem.url, 'page close done')
      ...
      logger.info(jobItem.url, 'screenshot created')
      if (jobItem.local) {
      ...
      logger.info(jobItem.url, 'uploadS3 done')

      await thumbnail.generateImageThumbnail(filename, thumbnailFilepath).catch((err) => {
        throw new Error('Can\'t generate thumbnail: ' + err.name + ': ' + (err && Object.hasOwn(err, 'message')) ? err.message : err)
      })

      logger.info(jobItem.url, 'generateImageThumbnail done')

      const s3UrlThumbnail = await uploadS3.upload(thumbnailFilepath).catch((err) => {
        throw new Error('Can\'t upload thumbnail: ' + err.name + ': ' + (err && Object.hasOwn(err, 'message')) ? err.message : err)
      })

      logger.info(jobItem.url, 'uploadS3Thumbnail done')
      ...
      logger.info(jobItem.url, 'uploadHtmlFileString done')
      ...
      logger.info(jobItem.url, 'uploadJsConsoleFileString done')
      ...
      logger.info(jobItem.url, 'sendResult')
