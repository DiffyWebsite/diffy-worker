  ...
  perform: async (browser, job, jobItem) => {
    ...
    while (attempt < maxAttempts) {
      ...
      try {
        ...
        const s3HtmlUrl = await uploadS3.uploadFileString(htmlFilename, pageHtml, uploadS3.htmlStorageClass).catch((err) => {
        ...
        if (mhtmlFilename) {
          s3MhtmlUrl = await uploadS3.uploadFileString(mhtmlFilename, pageMhtml, uploadS3.htmlStorageClass).catch((err) => {
