...
const debugLog = (data, jobItem = {}, additional = null, includeKey = false) => {
  if (debug) {
    ...
    if (includeKey) { // 'data' can be type of object
      if (additional) {
        logger.info(key, data, additional)
      } else {
        logger.info(key, data)
      }
    } else { // 'data' should be type of string
      if (additional) {
        logger.info(data, additional)
      } else {
        logger.info(data)
      }
    }
...
module.exports = {
  ...
  debugLog: (data, jobItem = {}, additional = null, includeKey = false) => {
    return debugLog(data, jobItem, additional, includeKey)
  }
}
