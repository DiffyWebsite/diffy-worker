  ...
  perform: async (browser, job, jobItem) => {
    ...
    while (attempt < maxAttempts) {
      ...
      try {
        ...
        try {
          ensureOpen(page, 'freezeJsTimers');
          await page.evaluate(() => {
            const highId = window.setTimeout(() => {}, 0)
            for (let i = 1; i <= highId; i++) {
          ...
          logger.warn('Failed to freeze JS timers', {error: e})
        }

        try {
          ensureOpen(page, 'resetCarouselPositions');
          await page.evaluate(() => {
            document.querySelectorAll('[style*="translate("]').forEach(el => {
              const transform = el.style.transform || ''
              if (/translate\s*\(-\d/.test(transform)) {
                el.style.transform = 'translate(0px, 0px)'
              }
            })
          })
        } catch (e) {
          logger.warn('Failed to reset carousel positions', {error: e})
        }
        ...
        try {
          ensureOpen(page, 'forceEagerImages');
          await page.evaluate(() => {
          ...
          logger.warn('Failed to force eager image loading', {error: e})
        }
