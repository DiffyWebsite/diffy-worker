  ...
  perform: async (browser, job, jobItem) => {
    ...
    while (attempt < maxAttempts) {
      ...
      try {
        ...
        // Keep a normal viewport before stabilization records element heights.
        // The final full-page resize happens shortly before the screenshot.
        ...
        if (stretchedImagesStabilizationEnabled && !page.isClosed()) {
          const fixedCount2 = await func.fixViewportRelativeHeights(page, true)
          if (fixedCount2 > 0) {
            if (!page.isClosed()) {
              await page.setViewportSize({width: parseInt(jobItem.breakpoint), height: 100})
              await page.waitForTimeout(1000)
              ...
              data.pageArea = pageHeight * jobItem.breakpoint
              logger.debug('updatePageViewport after stretched elements fix done', {page_height: pageHeight})
            }
          }
        }
