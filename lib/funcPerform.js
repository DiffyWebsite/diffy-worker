  ...
  perform: async (browser, job, jobItem) => {
    ...
    try {
      ...
      try {
        ...
        await page.waitForSelector('.header.mti_font_element', { timeout: 60000 })
      } catch (err) {
        logger.warn(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, 'page was not loaded by networkidle2');

        try {
          await page.goto(url, { timeout: 180000, waitUntil: ['load', 'networkidle0', 'domcontentloaded'] });
          await page.waitForSelector('.header.mti_font_element', { timeout: 60000 })
        } catch (err) {
          logger.error(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, 'page was not loaded by networkidle0 or domcontentloaded', err);
        }
      }
