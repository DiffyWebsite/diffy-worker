...
module.exports = {
  ...
  perform: async (browser, job, jobItem) => {
    ...
    try {
      ...
      if (Object.hasOwn(jobItem, 'cache_fonts') && jobItem.cache_fonts) {
        let cache = {};
        if (fs.existsSync(cachePath)) {
          try {
            ...
            if (cacheData.trim() !== "") {
              ...
              logger.info(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, 'Warning: cache.json is empty, initializing empty cache');
            }
          } catch (error) {
            logger.error(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, 'Error parsing cache.json', error)
            cache = {};
          }
        }
        ...
        page.on('request', (request) => {
          ...
          if ((url.includes('fonts.googleapis.com') || url.includes('fonts.gstatic.com')) && cache[url]) {
            request.respond({
        ...
        page.on('response', async (response) => {
          ...
          if ((url.includes('fonts.googleapis.com') || url.includes('fonts.gstatic.com')) && !cache[url] && response.ok()) {
            try {
              cache[url] = {
              ...
              logger.info(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, 'Successfully updated cache.json');
            } catch (error) {
              logger.error(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, 'Error writing to cache.json', error);
            }
          }
        });
      }
