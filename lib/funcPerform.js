      ...
      try {
        ...
        if (!page.isClosed()) {
          try {
            ...
            const cx = Math.floor((vp?.width || parseInt(jobItem.breakpoint) || 1200) / 2)
            const cy = Math.floor((vp?.height || 1000) / 2)
            await page.mouse.move(cx, cy)

            const scrollHeight = await page.evaluate(() => document.documentElement.scrollHeight)
            const step = 300
            const steps = Math.min(Math.ceil(scrollHeight / step) + 5, 300)
