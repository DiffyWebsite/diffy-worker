...
const stabilizationUrl = 'https://app.diffy.website/stabilization.js';
...
const saveError = async (job, jobItem, errorText) => {
  ...
  try {
    ...
    const width = (jobItem && jobItem.hasOwnProperty('breakpoint')) ? jobItem.breakpoint : 1024
...
async function loadAndRunStabilization(page, func) {
  try {
    const response = await axios.get(stabilizationUrl);
    const scriptContent = response.data;

    const exports = {};
    const module = { exports };

    const scriptFunction = new Function('page', 'func', 'module', 'exports', `${scriptContent}; return module.exports;`);

    const stabilize = scriptFunction(page, func, module, exports);

    if (typeof stabilize === 'function') {
      await stabilize(page, func);
    } else {
      console.error('Stabilization function is not defined');
    }
  } catch (error) {
    console.error('Failed to load or run stabilization script:', error);
    throw error;
  }
}
...
module.exports = {
  ...
  perform: async (browser, job, jobItem) => {
    ...
    try {
      ...
      let url = jobItem.url.replaceAll(parameters_regex, '');
      ...
      if (jobItem.args.hasOwnProperty('stabilization') && jobItem.args.stabilization) {
        await loadAndRunStabilization(page, func);
      }
