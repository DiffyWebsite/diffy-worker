  ...
  perform: async (browser, job, jobItem) => {
    ...
    try {
      ...
      if (Object.hasOwn(jobItem, 'cache_fonts') && jobItem.cache_fonts) {
        ...
        page.on('response', async (response) => {
          ...
          if (url.endsWith('.css') && response.ok()) {
            ...
            if (cachedFile) {
              logger.info(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, `CSS already cached: ${fileName}`);
              return;
            }
            ...
            try {
              ...
              logger.info(
                  ...
                  `Successfully cached CSS: ${fileName} with Content-Type: ${contentType}`
              );
            } catch (error) {
              logger.error(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, 'Error caching CSS response', error);
            }
            ...
            if (cachedFontFile) {
              logger.info(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, `Font already cached: ${fontName}`);
              return;
            }
            ...
            try {
              ...
              logger.info(
                  jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url,
                  `Successfully cached font: ${fontName} with Content-Type: ${contentType}`
              );
            } catch (error) {
              logger.error(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, 'Error caching font response', error);
            }
          }
        });
      }
      ...
      try {
        ...
        logger.error(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, 'page was not loaded by networkidle2')
      ...
      await page.setViewport({ width: parseInt(jobItem.breakpoint), height: 1000, deviceScaleFactor: 1 })
      await new Promise(resolve => setTimeout(resolve, 1000));
      ...
      if (is_cut) {
        ...
        await page.setViewport({ width: parseInt(jobItem.breakpoint), height: 100, deviceScaleFactor: 1, })
