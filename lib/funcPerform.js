...
const checkUrl = async (url, job) => {
  ...
  if (job.args.url) {
    try {
      ...
      logger.error('Failed to checkUrl (get, auth)', { url: job.args.url, e });
      lastException = e;
    }
  }
...
module.exports = {
  ...
  perform: async (browser, job, jobItem) => {
    ...
    try {
      ...
      if (authCookies) {
        logger.info('authCookies', { authCookies })
      ...
      if (Object.hasOwn(jobItem, 'project_id') && jobItem.project_id === 21791) {
        ...
        logger.info('Apply calltrkNoswap')
      ...
      try {
        ...
        logger.warning('page was not loaded by networkidle2')

        try {
          await page.goto(url, { timeout: 120000, waitUntil: ['load', 'domcontentloaded'] })
        } catch (err) {
          logger.error('page was not loaded by load or domcontentloaded')
        }
      }
      ...
      if (Object.hasOwn(jobItem.args, 'disable_css_animation') && jobItem.args.disable_css_animation) {
        logger.info('disable css animation')
        ...
        await page.addStyleTag({
          ...
          `
        }).catch((e) => logger.warning('Failed to add style tag', { error: e }))

        try {
          await disableGifAnimation(page)
        } catch (e) {
          logger.warning('Failed to disable GIF animation', { error: e })
        }
      }
      ...
      logger.info('updatePageViewport done', { page_height })
      ...
      logger.info('addJsCode done')

      logger.info('delayBeforeScreenshot done')
      ...
      logger.info('cutElements done')

      await func.addFixtures(page, jobItem)
      logger.info('addFixtures done')

      await func.hideBanners(page, jobItem)
      logger.info('hideBanners done')
      ...
      logger.info('uploadS3 done')

      await thumbnail.generateImageThumbnail(filename, thumbnailFilepath).catch((err) => {
        throw new Error('Can\'t generate thumbnail: ' + err.name + ': ' + (err && Object.hasOwn(err, 'message')) ? err.message : err)
      })

      logger.info('generateImageThumbnail done')

      const s3UrlThumbnail = await uploadS3.upload(thumbnailFilepath).catch((err) => {
        throw new Error('Can\'t upload thumbnail: ' + err.name + ': ' + (err && Object.hasOwn(err, 'message')) ? err.message : err)
      })

      logger.info('uploadS3Thumbnail done')
      ...
      logger.info('uploadHtmlFileString done')
      ...
      if (mhtmlFilename) {
        ...
        logger.info('uploadMhtmlFileString done', { job_item: jobItem })
      }
      ...
      logger.info('uploadJsConsoleFileString done')
