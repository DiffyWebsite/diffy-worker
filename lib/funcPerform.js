  ...
  perform: async (browser, job, jobItem) => {
    ...
    try {
      ...
      page.on('request', async (request) => {
        ...
        if (fs.existsSync(filePath)) {
          try {
            ...
            logger.info(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, `Serving cached file: ${fileName}`);
          } catch (error) {
            logger.warn(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, `Failed to read cached file: ${filePath}`, error);
          }
        }
      ...
      page.on('response', async (response) => {
        ...
        if ((fontExtensions.includes(extension) || url.endsWith('.css')) && response.ok()) {
          if (fs.existsSync(filePath)) {
            logger.info(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, `File already cached: ${fileName}`);
          }
          ...
          try {
            ...
            logger.info(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, `Cached file: ${fileName}`);
          } catch (error) {
            logger.warn(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, `Error caching file: ${fileName}`, error);
          }
        }
      });
