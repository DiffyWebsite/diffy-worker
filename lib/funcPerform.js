  ...
  perform: async (browser, job, jobItem) => {
    ...
    try {
      ...
      if (Object.hasOwn(jobItem, 'cache_fonts') && jobItem.cache_fonts) {
        const cacheDir = path.resolve(__dirname, '/tmp/fonts/');
        ...
        page.on('request', (request) => {
          ...
          if ((url.includes('fonts.googleapis.com') || url.includes('fonts.gstatic.com')) && cachedFile) {
        ...
        page.on('response', async (response) => {
          ...
          if ((url.includes('fonts.googleapis.com') || url.includes('fonts.gstatic.com')) && response.ok()) {
            const files = fs.readdirSync(cacheDir);
            const cachedFile = files.find(file => file.startsWith(fontName));
