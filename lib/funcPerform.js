  ...
  perform: async (browser, job, jobItem) => {
    ...
    while (attempt < maxAttempts) {
      ...
      try {
        ...
        if (!page.isClosed()) {
          const verticalShiftReport = await safeEval(page, () => {
            const BOUNDARY_BAND = 0.1 // px - how close to the .5 boundary counts as risky
            const MIN_HEIGHT = 50
            const MIN_WIDTH_RATIO = 0.5
            const MAX_SIGNIFICANT = 2000
            ...
            for (const element of document.querySelectorAll('body *')) {
              ...
              const isSignificant = (
                ...
                viewportWidth > 0 &&
                rect.width >= viewportWidth * MIN_WIDTH_RATIO
              )
              ...
              checked++
              const offset = Math.abs(Math.round(rect.top) - rect.top)
              const distanceToBoundary = 0.5 - offset

              if (distanceToBoundary <= BOUNDARY_BAND) {
                shifted++
                if (offset > maxOffset) {
                  maxOffset = offset
                }
              ...
              if (checked >= MAX_SIGNIFICANT) {
                capped = true
                break
              }
            }

            return { scanned, checked, shifted, maxOffset: Number(maxOffset.toFixed(3)), samples, capped }
          }, undefined, 'detect sub-pixel vertical shift').catch((e) => {
            logger.warn('Failed to detect sub-pixel vertical shift', { error: e?.message || String(e) })
            return null
          })

          if (verticalShiftReport && verticalShiftReport.shifted > 0) {
            logger.warn('Detected sub-pixel vertical shift bug', {
              project_id: jobItem.project_id,
              url: jobItem.url,
              breakpoint: jobItem.breakpoint,
              scanned: verticalShiftReport.scanned,
              checked: verticalShiftReport.checked,
              shifted: verticalShiftReport.shifted,
              maxOffset: verticalShiftReport.maxOffset,
              samples: verticalShiftReport.samples,
              capped: verticalShiftReport.capped,
            })
          }
        }
