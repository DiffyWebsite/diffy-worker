...
module.exports = {
  ...
  perform: async (browser, job, jobItem) => {
    ...
    try {
      ...
      logger.info(jobItem.url,'browser.newPage', jobItem)

      // set global timeout and disable CSP
      await page.setBypassCSP(true)
      await page.setDefaultNavigationTimeout(90 * 1000)
      logger.info(jobItem.url, 'setDefaultNavigationTimeout done', jobItem)
      page.on('console', msg => {
        let consoleMes
        try {
          consoleMes = {
      ...
      logger.info(jobItem.url, 'setHeaders done', jobItem)
      ...
      logger.info(jobItem.url, 'addCookies done', jobItem)
      ...
      logger.info(jobItem.url, 'auth done', jobItem)
      if (authCookies) {
        logger.info(jobItem.url, 'authCookies', jobItem, authCookies)
      ...
      if (Object.hasOwn(jobItem, 'project_id') && jobItem.project_id === 21791) {
        ...
        logger.info(jobItem.url, 'Apply calltrkNoswap')
      ...
      try {
        ...
        logger.info(jobItem.url, 'page was not loaded by networkidle2', jobItem)
        await page.goto(url, { timeout: 60000, waitUntil: ['domcontentloaded'] })
      }
      logger.info(jobItem.url, 'page loaded done', jobItem)
      ...
      if (Object.hasOwn(jobItem.args, 'disable_css_animation') && jobItem.args.disable_css_animation) {
        logger.info(jobItem.url, 'disable css animation', jobItem)
        ...
        await page.addStyleTag({
          ...
          `
        }).catch((e) => logger.error(jobItem.url, 'Failed to add style tag', e))

        try {
          await disableGifAnimation(page)
        } catch (e) {
          logger.error(jobItem.url, 'Failed to disable GIF animation', e)
        }
      }

      await page.setViewport({ width: parseInt(jobItem.breakpoint), height: 1000 })
      await page.waitForTimeout(1000)
      logger.info(jobItem.url, 'page.goto done', jobItem)

      logger.startTimer('waitFontsReady', jobItem.url);
      await page.evaluateHandle('document.fonts.ready');
      logger.endTimer('waitFontsReady', jobItem.url);
      ...
      logger.info(jobItem.url, 'addCssCode done', jobItem)
      ...
      logger.info(jobItem.url, 'autoScroll done', jobItem)
      ...
      logger.info(jobItem.url, 'updatePageViewport done', jobItem, { page_height })
      ...
      logger.info(jobItem.url, 'addJsCode done', jobItem)

      logger.info(jobItem.url, 'delayBeforeScreenshot done', jobItem)
      ...
      logger.info(jobItem.url, 'cutElements done', jobItem)
      await func.hideBanners(page, jobItem)
      logger.info(jobItem.url, 'hideBanners done', jobItem)

      await func.addFixtures(page, jobItem)
      logger.info(jobItem.url, 'addFixtures done', jobItem)
      ...
      logger.info(jobItem.url, 'start screenshot', jobItem)
      ...
      logger.info(jobItem.url, 'screenshot done', jobItem)
      const pageHtml = await func.getPageHtml(page)
      logger.info(jobItem.url, 'pageHtml done', jobItem)
      if (is_crop) {
      ...
      await page.close()
      logger.info(jobItem.url, 'page close done', jobItem)
      ...
      logger.info(jobItem.url, 'screenshot created', jobItem)
      if (jobItem.local) {
        const fs = require('node:fs');
        try {
          fs.writeFileSync(htmlFilename, pageHtml);
        } catch (err) {
          logger.error(jobItem.url, 'Failed to write file', err);
        }
        try {
      ...
      const s3Url = await uploadS3.upload(filename).catch((err) => {
        logger.error(jobItem.url, 'Failed to upload file to S3', err);
        throw new Error('Can\'t upload screenshot: ' + err.name + ': ' + (err && Object.hasOwn(err, 'message')) ? err.message : err)
      })

      logger.info(jobItem.url, 'uploadS3 done', jobItem)

      await thumbnail.generateImageThumbnail(filename, thumbnailFilepath).catch((err) => {
        throw new Error('Can\'t generate thumbnail: ' + err.name + ': ' + (err && Object.hasOwn(err, 'message')) ? err.message : err)
      })

      logger.info(jobItem.url, 'generateImageThumbnail done', jobItem)

      const s3UrlThumbnail = await uploadS3.upload(thumbnailFilepath).catch((err) => {
        throw new Error('Can\'t upload thumbnail: ' + err.name + ': ' + (err && Object.hasOwn(err, 'message')) ? err.message : err)
      })

      logger.info(jobItem.url, 'uploadS3Thumbnail done', jobItem)
      ...
      logger.info(jobItem.url, 'uploadHtmlFileString done', jobItem)
      ...
      logger.info(jobItem.url, 'uploadJsConsoleFileString done', jobItem)
      ...
      logger.info(jobItem.url, 'sendResult', jobItem)
      ...
      logger.error(jobItem.url, 'perform error:', err)

      if (page !== null) {
        try {
          await page.close()
        } catch (e) {
          logger.error(jobItem.url, 'Failed to close page', e)
        }
      }
