  ...
  perform: async (browser, job, jobItem) => {
    ...
    while (attempt < maxAttempts) {
      ...
      try {
        ...
        try {
          await page.evaluate(() => {
            // Stop Webflow IX2 event listeners
            if (window.Webflow && typeof window.Webflow.destroy === 'function') {
              window.Webflow.destroy()
            }

            // Kill GSAP animations — Webflow IX2 drives transforms/opacity via GSAP's
            // own RAF loop, which continues running after Webflow.destroy()
            if (window.gsap) {
              window.gsap.globalTimeline.pause(0)
              window.gsap.killAll()
            }

            // Reset elements left mid-animation by GSAP after kill.
            // GSAP 3 signature: individual CSS transform props (translate/rotate/scale)
            // set to "none" alongside the legacy transform shorthand.
            document.querySelectorAll('[style]').forEach(el => {
              const s = el.style

              // Horizontal ticker/slider tracks: reset X translation to 0 so every run
              // starts from the same position instead of wherever the ticker happened to be
              if (s.translate === 'none' && s.rotate === 'none' && s.scale === 'none' && s.transform) {
                s.transform = s.transform.replace(
                  /translate3d\([^,]+,\s*0px,\s*0px\)/,
                  'translate3d(0px, 0px, 0px)'
                )
              }

              // Logo-ticker items animated with grayscale + opacity: force fully visible
              // so they don't differ based on where in the fade cycle they were frozen
              if (s.filter && s.filter.includes('grayscale') && s.opacity && parseFloat(s.opacity) < 1) {
                s.filter = s.filter.replace(/grayscale\([^)]+\)/, 'grayscale(0%)')
                s.opacity = '1'
              }
            })
          })
        } catch (e) {
          logger.warn('Failed to destroy Webflow/GSAP animations', {error: e})
        }
        ...
        // We need to decrease height after cut
        ...
        // Final determinism pass before screenshot:
        // 1. Kill all JS timers — stops counter animations and dynamic text updates
        //    that setInterval/setTimeout drive (CSS animation-duration:0s doesn't reach these).
        //    Safe at this point: page is loaded, fonts ready, autoScroll done.
        // 2. Round fractional inline pixel positions written by JS widgets (TrustIndex,
        //    Google Maps pins, chat launchers etc.) to prevent subpixel false diffs.
        try {
          await page.evaluate(() => {
            ...
            // Round fractional inline positions
            const props = ['top', 'left', 'right', 'bottom']
          ...
          logger.warn('Failed to run pre-screenshot determinism pass', {error: e})
        }
