  ...
  perform: async (browser, job, jobItem) => {
    ...
    try {
      ...
      if (jobItem.url && jobItem.base_url) {
        const pageUrl = new URL(jobItem.url);
        const baseUrl = new URL(jobItem.base_url);
      ...
      if (
        ...
        if (Object.hasOwn(jobItem, 'project_id') && jobItem.project_id === 1952 && job.environment === 'custom') {
          const base64 = Buffer.from(`${jobItem.basicAuth.user}:${jobItem.basicAuth.password}`).toString('base64');

          url = jobItem.url.replace(/^https:/, 'http:');
