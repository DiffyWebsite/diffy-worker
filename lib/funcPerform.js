...
// Embeds that size themselves from inside an iframe (JotForm widgets, iframe-resizer, HubSpot
// forms) measure their content at their own "ready" event and again only on window resize. When
// that measurement runs while their web font is still loading, the stale height is kept for good
// and everything below the embed shifts by a few pixels between captures. Wait for fonts in every
// child frame, then dispatch a resize event there so the embeds re-measure with final fonts.
// Every wait in here is capped, so the step adds at most fontsTimeoutMs + resizeTimeoutMs +
// settleMs to a capture and can never stall a job on a hung frame.
const settleChildFrames = async (page, {
  fontsTimeoutMs = 2000,
  resizeTimeoutMs = 2000,
  settleMs = 1000,
  label = 'settleChildFrames',
} = {}) => {
  ensureOpen(page, label)
  ...
  // No requestAnimationFrame here: hidden frames (tag managers, pixels) never paint, so a rAF
  // based wait would hang until the timeout.
  await Promise.all(frames.map((frame) => withTimeout(
  ...
  perform: async (browser, job, jobItem) => {
    ...
    while (attempt < maxAttempts) {
      ...
      try {
        ...
        // Custom headers are never installed globally; they are attached per request by the
        // route handler below, scoped to the job's own origins.
        ...
        logger.debug('headers prepared', {
          userAgent: userAgentString,
          originScopedHeaders: Object.keys(headerState?.customHeaders || {}),
          targetOrigins: headerState?.targetOrigins || []
        })
        ...
        const hasCustomHeaders = Boolean(headerState?.customHeaderNames?.length)
        const selectiveHeaderRoutingEnabled = callRailBlockEnabled || basicAuthRouteConfig || deterrenceState?.enabled || hasCustomHeaders
        ...
        if (selectiveHeaderRoutingEnabled) {
          await page.route('**/*', async (route) => {
            ...
            if (hasCustomHeaders) {
              if (func.shouldApplyCustomHeaders(headerState, requestUrl)) {
                func.applyCustomHeaders(headerState, ensureHeadersCopy());
              } else {
                // Cross-origin: a custom header here would force a CORS preflight that hosts
                // like fonts.gstatic.com reject, so make sure none is on the request.
                const leakedHeaders = func.findLeakedCustomHeaders(headerState, request.headers());
                if (leakedHeaders.length) {
                  const headers = ensureHeadersCopy();
                  leakedHeaders.forEach((name) => { delete headers[name]; });
                  logger.debug('Stripped custom headers from cross-origin request', {
                    requestUrl,
                    headers: leakedHeaders
                  });
                }
              }
            }
        ...
        // Custom CSS must be in place before the login page renders, otherwise an
        // overlay on it intercepts pointer events on the submit button.
        if (Object.hasOwn(jobItem, 'project_id') && jobItem.project_id === 32086) {
          await func.addCssCodeOnNavigation(page, jobItem)
          logger.debug('addCssCodeOnNavigation done')
        }
        ...
        if (!page.isClosed()) {
          try {
            const settledFrames = await settleChildFrames(page)
            logger.debug('settleChildFrames done', {frames: settledFrames})
          } catch (e) {
            logger.warn('settleChildFrames failed', {error: e?.message || String(e)})
          }
        }
