...
const stabilizeAnimatedTextPaint = async (page) => {
  return safeEval(page, () => {
    ...
    const hasInlineTranslateState = (element) => {
      if (!element?.style) {
        return false
      }

      return Array.from(element.style).some((name) => /^--.*translate/i.test(name)) ||
        element.style.transform.includes('translate')
    }

    const hasComputedVerticalTranslation = (computedTransform) => {
      if (!computedTransform || computedTransform === 'none') return false
      const m2d = computedTransform.match(/^matrix\(([^)]+)\)$/)
      if (m2d) {
        const vals = m2d[1].split(',').map(Number)
        return vals[5] !== 0
      }
      const m3d = computedTransform.match(/^matrix3d\(([^)]+)\)$/)
      if (m3d) {
        const vals = m3d[1].split(',').map(Number)
        return vals[13] !== 0
      }
      return false
    }
    ...
    document.querySelectorAll('*').forEach((element) => {
      ...
      if (computedAspectRatio && computedAspectRatio !== 'auto' && computedAspectRatio !== '') {
        if (rect.height > 0) {
      ...
      if (!hasDirectTextNode(element)) {
        const shouldReset = hasInlineTranslateState(element) || hasComputedVerticalTranslation(style.transform)
        ...
        if (shouldReset) {
          ...
          Array.from(element.style)
            .filter((name) => /^--.*translate/i.test(name))
            .forEach((name) => {
              element.style.setProperty(name, '0px', 'important')
            })
        }
      }
    })
