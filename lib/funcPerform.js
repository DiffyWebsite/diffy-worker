...
module.exports = {
  ...
  perform: async (browser, job, jobItem) => {
    ...
    try {
      ...
      try {
        response = await page.goto(url, { timeout: 120000, waitUntil: ['networkidle2'] })
        ...
        try {
          response = await page.goto(url, { timeout: 120000, waitUntil: ['load', 'domcontentloaded'] })
        } catch (err) {
      ...
      await Promise.race([
        ...
        new Promise((_, rej) => setTimeout(() => rej(new Error('fonts.ready cap (6s)')), 6000)),
      ]).catch(e => logger.warn('fonts.ready skipped', { error: String(e) }));
      ...
      await page
          ...
          .catch(() => logger.warn('readyState=complete timed out'));
      await new Promise(resolve => setTimeout(resolve, 1000));
      ...
      await Promise.race([
        ...
        new Promise((_, rej) =>
            setTimeout(() => rej(new Error('touchstart poke timeout')), 3000)
        ),
      ]).catch(err => logger.warn('touchstart skipped', { error: String(err) }));
