  ...
  perform: async (browser, job, jobItem) => {
    ...
    try {
      ...
      try {
        ...
        try {
          await page.goto(url, { timeout: 120000, waitUntil: ['networkidle0, domcontentloaded'] })
        } catch (err) {
          logger.error(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, 'page was not loaded by load or domcontentloaded', err)
        }
      }
