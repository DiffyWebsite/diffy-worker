      ...
      try {
        ...
        if (recademicsMobileStabilizationEnabled && !page.isClosed()) {
          await safeEval(page, () => {
            ...
            document.querySelectorAll('.e-con.e-parent').forEach(snapToWholePixel)

            document.querySelectorAll('.e-n-accordion-item-title, .faq-text-block .elementor-widget-container')
              .forEach(snapToWholePixel)
          }, undefined, 'recademics section + faq text stabilization').catch((e) => {
            logger.warn('Failed to stabilize layout for project 26297', { error: e })
          })
