    ...
    try {
      ...
      if (Object.hasOwn(jobItem.args, 'stabilization') && jobItem.args.stabilization) {
        await page.evaluate(async () => {
          ...
          const stabilizeHeight = async (elementsHeights, level, logger) => {
            for (const element of elementsHeights) {
              if (document.body.contains(element.node)) {
                if (
                  ...
                  logger.info('[HeightStabilization] #' + level + ' Fixing: ', element.height, element.node.offsetHeight)
                ...
                if (element.childNodes.length) {
                  await stabilizeHeight(element.childNodes, level + 1, logger)
                }
              }
            }
          }

          await stabilizeHeight(window.diffyElementsHeights ?? [], 1, logger);
        }, )
