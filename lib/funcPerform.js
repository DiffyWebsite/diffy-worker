...
const checkUrl = async (url, job) => {
  ...
  try {
    ...
    logger.error(job.breakpoint + ':' + url, 'Failed to checkUrl', e);
    lastException = e;
  }
  ...
  try {
    ...
    logger.error(job.breakpoint + ':' + url, 'Failed to checkUrl (GET)', e);
    lastException = e;
  }
  ...
  if (job.args.url) {
    try {
      ...
      logger.error(job.breakpoint + ':' + job.args.url, 'Failed to checkUrl (get, auth)', e);
      lastException = e;
    }
  }
...
module.exports = {
  ...
  perform: async (browser, job, jobItem) => {
    ...
    // } catch (e) {
    //   debugLog('checkUrl error:', jobItem, e)
    ...
    try {
      ...
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'setDefaultNavigationTimeout done', jobItem)
      page.on('console', msg => {
        let consoleMes
        try {
          consoleMes = {
      ...
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'setHeaders done', jobItem)
      ...
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'addCookies done', jobItem)
      ...
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'auth done', jobItem)
      if (authCookies) {
        logger.info(jobItem.breakpoint + ':' + jobItem.url, 'authCookies', jobItem, authCookies)
      ...
      try {
        ...
        logger.info(jobItem.breakpoint + ':' + jobItem.url, 'page was not loaded by networkidle2', jobItem)
        await page.goto(url, { timeout: 60000, waitUntil: ['domcontentloaded'] })
      }
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'page loaded done', jobItem)
      ...
      if (Object.hasOwn(jobItem.args, 'disable_css_animation') && jobItem.args.disable_css_animation) {
        logger.info(jobItem.breakpoint + ':' + jobItem.url, 'disable css animation', jobItem)
      ...
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'page.goto done', jobItem)
      ...
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'addCssCode done', jobItem)
      ...
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'autoScroll done', jobItem)
      ...
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'updatePageViewport done', jobItem, { page_height })
      ...
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'addJsCode done', jobItem)

      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'delayBeforeScreenshot done', jobItem)
      ...
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'cutElements done', jobItem)
      await func.hideBanners(page, jobItem)
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'hideBanners done', jobItem)

      await func.addFixtures(page, jobItem)
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'addFixtures done', jobItem)
      ...
      await func.autoScroll(page, jobItem)
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'double autoScroll done', jobItem)
      const pageHeight = await func.updatePageViewport(page, jobItem, maxPageHeight)

      data.pageArea = pageHeight * jobItem.breakpoint

      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'updatePageViewport done', jobItem)

      const is_crop = await func.cropElement(page, jobItem)

      logger.info(jobItem.breakpoint + ':' + jobItem.url,'cropElement done', jobItem)
      ...
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'start screenshot', jobItem)
      ...
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'screenshot done', jobItem)
      const pageHtml = await func.getPageHtml(page)
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'pageHtml done', jobItem)
      if (is_crop) {
      ...
      await page.close()
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'page close done', jobItem)
      ...
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'screenshot created', jobItem)
      if (jobItem.local) {
      ...
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'uploadS3 done', jobItem)

      await thumbnail.generateImageThumbnail(filename, thumbnailFilepath).catch((err) => {
        throw new Error('Can\'t generate thumbnail: ' + err.name + ': ' + (err && Object.hasOwn(err, 'message')) ? err.message : err)
      })

      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'generateImageThumbnail done', jobItem)

      const s3UrlThumbnail = await uploadS3.upload(thumbnailFilepath).catch((err) => {
        throw new Error('Can\'t upload thumbnail: ' + err.name + ': ' + (err && Object.hasOwn(err, 'message')) ? err.message : err)
      })

      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'uploadS3Thumbnail done', jobItem)
      ...
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'uploadHtmlFileString done', jobItem)
      ...
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'uploadJsConsoleFileString done', jobItem)
      ...
      logger.info(jobItem.breakpoint + ':' + jobItem.url, 'sendResult', jobItem)
