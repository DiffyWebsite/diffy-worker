...
module.exports = {
  ...
  perform: async (browser, job, jobItem) => {
    ...
    try {
      ...
      await (async () => {
        const res = await fetch('https://cloud.typography.com/7959380/6269632/css/fonts.css');
        logger.error(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, await res.text())
        logger.error(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, res.status)
      })();
        ...
        if ((url.endsWith('.css') || url.includes('font')) && response.ok()) {
          ...
          try {
            ...
            if (contentLength && contentLength !== buffer.length) {
              logger.warn(
                  jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url,
                  `Skipping cache for ${url} because file size (${buffer.length}) ` +
                  `did not match Content-Length (${contentLength}).`
              );
              validFile = false;
            }
