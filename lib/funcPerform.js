...
const debugLog = (data, jobItem = {}, additional = false) => {
  if (debug) {
    ...
    if (additional) {
      console.log(key, data, additional) // Debug
    } else {
      console.log(key, data) // Debug
    }
  }
}
  ...
  perform: async (browser, job, jobItem) => {
    ...
    try {
      ...
      if (Object.hasOwn(jobItem, 'project_id') && jobItem.project_id === 21791) {
        ...
        logger.log('Apply calltrkNoswap')
            ...
            for (const element of elementsHeights) {
              if (document.body.contains(element.node)) {
                if (
                  ...
                  console.log('[HeightStabilization] #' + level + ' Fixing: ', element.height, element.node.offsetHeight) // Debug
