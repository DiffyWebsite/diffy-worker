...
function findCachedFile(cacheDir, baseName) {
  try {
    const files = fs.readdirSync(cacheDir);

    const matched = files.find((file) => file.startsWith(baseName + '___'));
    if (!matched) return null;

    const parts = matched.split('___');
    if (parts.length < 2) return null;

    const contentType = parts[1].replace(/_/g, '/');
    const filePath = path.join(cacheDir, matched);
...
function getCachedFileName(fileName, contentType = '') {
  const safeContentType = contentType.replace(/\//g, '_');
  return `${fileName}___${safeContentType}`;
}
...
module.exports = {
  ...
  perform: async (browser, job, jobItem) => {
    ...
    try {
      ...
      // Enhanced bot detection mitigation
      ...
      page.on('request', async (request) => {
        try {
          ...
          // We check CSS by extension .css
          if (url.endsWith('.css')) {
            const cached = findCachedFile(cacheDir, baseName);
            if (cached) {
              const fileBuffer = fs.readFileSync(cached.filePath);
          ...
          logger.warn(`${jobItem.id}:${jobItem.breakpoint}:${jobItem.url}`, 'Error in request interception:', error);
          request.continue();
        }
      });
      ...
      page.on('response', async (response) => {
        try {
          ...
          if (!response.ok()) {
            logger.error(`${jobItem.id}:${jobItem.breakpoint}:${jobItem.url}`, `Response error: ${baseName}`);
          }

          if (url.endsWith('.css')) {
            const cached = findCachedFile(cacheDir, baseName);
            if (cached) {
              logger.info(`${jobItem.id}:${jobItem.breakpoint}:${jobItem.url}`, `CSS already cached: ${baseName}`);
            }

            // Cache it
            const cssBuffer = await response.buffer();
            const rawContentType = response.headers()['content-type'] || 'text/css';
            const cachedFileName = getCachedFileName(baseName, rawContentType);
            const cssPath = path.join(cacheDir, cachedFileName);

            fs.writeFileSync(cssPath, cssBuffer);
            logger.info(
                `${jobItem.id}:${jobItem.breakpoint}:${jobItem.url}`,
                `Successfully cached CSS: ${baseName} with Content-Type: ${rawContentType}`
            );
          }

          // Similarly, handle font files if the URL includes 'font'
          else if (url.includes('font')) {
            const cached = findCachedFile(cacheDir, baseName);
            if (cached) {
              logger.info(`${jobItem.id}:${jobItem.breakpoint}:${jobItem.url}`, `Font already cached: ${baseName}`);
            }

            // Cache the font
            const fontBuffer = await response.buffer();
            const rawContentType = response.headers()['content-type'] || 'application/font-woff';
            const cachedFileName = getCachedFileName(baseName, rawContentType);
            const fontPath = path.join(cacheDir, cachedFileName);

            fs.writeFileSync(fontPath, fontBuffer);
            logger.info(
                `${jobItem.id}:${jobItem.breakpoint}:${jobItem.url}`,
                `Successfully cached font: ${baseName} with Content-Type: ${rawContentType}`
            );
          }
        } catch (error) {
          logger.warn(`${jobItem.id}:${jobItem.breakpoint}:${jobItem.url}`, 'Error in response handler:', error);
        }
      });
