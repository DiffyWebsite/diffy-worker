...
const checkUrl = async (url, job) => {
  ...
  try {
    ...
    logger.error('Failed to checkUrl', e);
    lastException = e;
  }
  ...
  try {
    ...
    logger.error('Failed to checkUrl (GET)', e);
    lastException = e;
  }
  ...
  if (job.args.url) {
    try {
      ...
      logger.error('Failed to checkUrl (get, auth)', e);
      lastException = e;
    }
  }
...
const debugLog = (data, jobItem = {}, additional = null, finishJob = false) => {
  if (debug) {
    ...
    let logs = debugLogsMap.get(url)
    if (logs) {
      logs.push(log)
    } else {
      logs = [ log ]
    }
    debugLogsMap.set(url, logs)

    if (finishJob) {
      logger.info(key, logs)
      debugLogsMap.delete(url)
    }
  }
}
...
const saveError = async (job, jobItem, errorText) => {
  ...
  try {
    ...
    // Not need to remove "mhtmlFilename" because we use stream and not creating real file.
    // Async remove files.
...
module.exports = {
  ...
  perform: async (browser, job, jobItem) => {
    ...
    // debugLog('Start process:', jobItem, job) display a lot of info
    // try {
    ...
    try {
      ...
      if (authCookies) {
        debugLog('authCookies', jobItem, authCookies)
      ...
      if (Object.hasOwn(jobItem.args, 'disable_css_animation') && jobItem.args.disable_css_animation) {
        ...
        await page.addStyleTag({
          ...
          `
        }).catch((e) => logger.error('Failed to add style tag', e))

        try {
          await disableGifAnimation(page)
        } catch (e) {
          logger.error('Failed to disable GIF animation', e)
        }
      }
      ...
      if (jobItem.local) {
        ...
        try {
          ...
          logger.error('Failed to write HTML file', err);
        }
        if (mhtmlFilename) {
          try {
            fs.writeFileSync(mhtmlFilename, pageMhtml);
          } catch (err) {
            logger.error('Failed to write MHTML file', err);
          }
        }
      ...
      if (mhtmlFilename) {
        const s3MhtmlUrl = await uploadS3.uploadFileString(mhtmlFilename, pageMhtml).catch((err) => {
          throw new Error('Can\'t upload mhtml file: ' + err.name + ': ' + (err && Object.hasOwn(err, 'message')) ? err.message : err)
        })
        debugLog('uploadMhtmlFileString done', jobItem)
      }
      else {
        const s3MhtmlUrl = ''
      }
