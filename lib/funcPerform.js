...
async function waitForFontsToLoadWithRetry(page, maxRetries = 3, delay = 1000) {
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      console.log(`Retrying to load fonts... Attempt ${i + 1}`);
      await page.waitForTimeout(delay);
    }
  }
  throw new Error('Failed to load fonts after multiple attempts');
}
...
module.exports = {
  ...
  perform: async (browser, job, jobItem) => {
    ...
    try {
      ...
      await waitForFontsToLoadWithRetry(page);
