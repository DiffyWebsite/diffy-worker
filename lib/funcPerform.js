    ...
    try {
      ...
      if (fs.existsSync(cachePath)) {
        try {
          ...
          if (cacheData.trim() !== "") { // Ensure the file is not empty
            cache = JSON.parse(cacheData);
          } else {
            console.warn(`[${new Date().toISOString()}] Warning: cache.json is empty, initializing empty cache.`);
            cache = {};
          }
        } catch (error) {
          console.error(`[${new Date().toISOString()}] Error parsing cache.json:`, error);
          cache = {}; // Reinitialize to an empty object if parsing fails
          fs.writeFileSync(cachePath, '{}'); // Reset cache.json if corrupted
        }
      }
      ...
      page.on('request', (request) => {
        ...
        if (url.includes('font') && cache[url]) {
          request.respond({
            status: cache[url].status,
            contentType: cache[url].contentType,
      ...
      page.on('response', async (response) => {
        ...
        if (url.includes('font') && !cache[url] && response.ok()) {
          cache[url] = {
          ...
          const tempPath = path.join(__dirname, 'cache.json.tmp');
          try {
            if (!fs.existsSync(path.dirname(cachePath))) {
              console.log(`[${new Date().toISOString()}] Creating directory: ${path.dirname(cachePath)}`);
              fs.mkdirSync(path.dirname(cachePath), { recursive: true });
            }

            console.log(`[${new Date().toISOString()}] Writing to temporary cache file: ${tempPath}`);
            fs.writeFileSync(tempPath, JSON.stringify(cache, null, 2));

            console.log(`[${new Date().toISOString()}] Renaming ${tempPath} to ${cachePath}`);
            fs.renameSync(tempPath, cachePath);

            console.log(`[${new Date().toISOString()}] Successfully updated cache.json`);
          } catch (error) {
            console.error(`[${new Date().toISOString()}] Error writing to cache.json:`, error);
      ...
      try {
        ...
        logger.warn(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, 'page was not loaded by networkidle2', err)

        try {
          await page.goto(url, { timeout: 60000, waitUntil: ['load', 'domcontentloaded'] })
        } catch (err) {
          logger.error(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, 'page was not loaded by load and domcontentloaded', err)
        }
      }
      ...
      if (Object.hasOwn(jobItem.args, 'disable_css_animation') && jobItem.args.disable_css_animation) {
        ...
        await page.addStyleTag({
          ...
          `
        }).catch((e) => logger.error(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, 'Failed to add style tag', e))

        try {
          await disableGifAnimation(page)
        } catch (e) {
          logger.error(jobItem.id + ':' + jobItem.breakpoint + ':' + jobItem.url, 'Failed to disable GIF animation', e)
        }
      }
