...
const isCertificateInterstitial = async (page) => {
  try {
    ...
    return normalizedContent.includes('your connection is not private') ||
      normalizedContent.includes('attackers might be trying to steal your information') ||
      normalizedContent.includes('net::err_cert');
  } catch (error) {
    logger.warn('Failed to detect certificate interstitial', { error });
    return false;
  }
};

const bypassCertificateInterstitial = async (page, url, timeoutMs) => {
  const bypassLogContext = { targetUrl: url };

  if (!(await isCertificateInterstitial(page))) {
    return false;
  }

  logger.warn('Certificate interstitial detected, attempting bypass', bypassLogContext);

  try {
    await page.keyboard.type('thisisunsafe');
  } catch (error) {
    logger.error('Failed to type interstitial bypass command', { error, ...bypassLogContext });
    return false;
  }
  ...
  try {
    ...
    logger.warn('Navigation wait failed after certificate bypass attempt', { error, ...bypassLogContext });
  }

  await new Promise(resolve => setTimeout(resolve, 500));

  if (await isCertificateInterstitial(page)) {
    logger.error('Certificate interstitial bypass unsuccessful', bypassLogContext);
    return false;
  }

  logger.info('Certificate interstitial bypassed successfully', bypassLogContext);
  return true;
};
...
module.exports = {
  ...
  perform: async (browser, job, jobItem) => {
    ...
    try {
      ...
      const needIncrease = jobItem?.args?.need_increase;
      const timeouts = {
        default: resolveTimeoutWithNeedIncrease(needIncrease, 30000),
        navigation: resolveTimeoutWithNeedIncrease(needIncrease, 90 * 1000),
        goto: resolveTimeoutWithNeedIncrease(needIncrease, 120000),
      };
      ...
      await page.setDefaultTimeout(timeouts.default)
      ...
      if (
        ...
        if (url.startsWith('http://')) {
          const base64 = Buffer.from(`${jobItem.basicAuth.user}:${jobItem.basicAuth.password}`).toString('base64');
          const targetHost = new URL(jobItem.base_url).host;
          ...
          page.on('request', (request) => {
            ...
            // Only downgrade to HTTP if host matches base_url
            if (currentHost === targetHost) {
              reqUrl = reqUrl.replace(/^https:/, 'http:');
            }

            request.continue({ headers, url: reqUrl });
          });
      ...
      try {
        response = await page.goto(url, { timeout: timeouts.goto, waitUntil: ['networkidle2'] })
        ...
        try {
          response = await page.goto(url, { timeout: timeouts.goto, waitUntil: ['load', 'domcontentloaded'] })
        } catch (err) {
          logger.error('page was not loaded by load or domcontentloaded', { error: err, url })
        }
      }

      const certificateBypassed = await bypassCertificateInterstitial(page, url, timeouts.goto);
      if (certificateBypassed) {
        await new Promise(resolve => setTimeout(resolve, 1000));
      } else if (await isCertificateInterstitial(page)) {
        throw new Error('Unable to bypass certificate interstitial');
      }
