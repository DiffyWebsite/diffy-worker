  ...
  perform: async (browser, job, jobItem) => {
    ...
    while (attempt < maxAttempts) {
      ...
      try {
        ...
        // Always scroll through the page to trigger lazy-loaded content (IntersectionObserver,
        // scroll-event listeners, Slick carousel init) regardless of scroll_step config.
        // Re-reads scrollHeight at each step so content added during scrolling is also reached.
        if (!page.isClosed()) {
          try {
            ...
            logger.warn('Lazy-load scroll trigger failed', { error: e?.message || String(e) })
          }
        }
