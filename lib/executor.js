...
class Executor {
  ...
  async run (browser, data) {
    ...
    try {
      ...
      if (result?.status) {
        funcPerform.debugLog('Run success result', data.params, result, true)
        await this.sqsSender.sendSQSResult(result, timeExecuteStart)
      } else {
        funcPerform.debugLog('Run error result', data.params, result, true)
        await this.sqsSender.resend(data, timeExecuteStart, Object.hasOwn(result, 'err') ? result.err : result)
      }
      return result
    } catch (e) {
      if (this.local) {
        this.logger.error(data.params.breakpoint + ':' + data.params.url, 'Failed to run executor', e)
        return result
      }

      funcPerform.debugLog('Run error result', data.params, e, true)
  ...
  async timeout (handlerTimeExecuteStart) {
    this.logger.info(`timeout: ${this.item}`)
    ...
    if (this.item?.attempts && this.item.attempts < maxAttempts) {
      ...
      try {
        ...
        this.logger.error('Failed to stringify data', e)
      }
