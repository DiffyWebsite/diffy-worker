...
class Jobs {
  ...
  prepareJobs (screenshotBaseUrl, project) {
    ...
    this.logger.debug(`Screenshot ${screenshotUris.length} URLs`)
    ...
    this.logger.debug('Breakpoints', { breakpoints })
    ...
    this.logger.debug(`Prepared ${jobs.length} jobs`)
  ...
  _getAdvancedArgs (project) {
    ...
    // Add stretched images stabilization.
    if (Object.hasOwn(advanced, 'psStretchedImagesStabilization') && advanced.psStretchedImagesStabilization) {
      advancedArgs.stretched_images_stabilization = true
    }
    ...
    // Add javascript code. psScreenshotJsCode is either a single string (old
    // style) or an array of blocks (new style). Normalize to trimmed, non-empty
    // blocks and pass a single block as a string for backward compatibility,
    // multiple as an array so each runs in isolation (see func.addJsCode).
    if (Object.hasOwn(advanced, 'psScreenshotJs') && advanced.psScreenshotJs &&
      Object.hasOwn(advanced, 'psScreenshotJsCode')) {
      const rawBlocks = Array.isArray(advanced.psScreenshotJsCode)
        ? advanced.psScreenshotJsCode
        : [advanced.psScreenshotJsCode]
      const jsBlocks = rawBlocks
        .filter(block => typeof block === 'string')
        .map(block => block.trim())
        .filter(block => block.length)

      if (jsBlocks.length === 1) {
        advancedArgs.js_code = jsBlocks[0]
      } else if (jsBlocks.length > 1) {
        advancedArgs.js_code = jsBlocks
      }
    }
