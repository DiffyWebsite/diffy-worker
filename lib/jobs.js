...
class Jobs {
  ...
  prepareJobs (screenshotBaseUrl, project) {
    ...
    // project.production may carry a trailing slash while project.urls entries don't
    // (e.g. production "https://example.com/" vs root url "https://example.com"). Left
    // untrimmed, the replace() below fails to match the root url at all (leaving its uri as
    // the full URL) and eats the leading slash off every other page's uri. Trim both sides
    // the same way so the prefix always matches.
    projectBaseDomain = this._rTrim(projectBaseDomain)
