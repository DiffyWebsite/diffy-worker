...
class ChromiumBrowser {
  ...
  async getBrowser (proxy, options = {}) {
    ...
    const handlerTimeout = typeof options?.handlerTimeout === 'number' && Number.isFinite(options.handlerTimeout)
      ? Math.max(options.handlerTimeout, 180000)
      : 180000;
    const protocolTimeout = resolveTimeoutWithNeedIncrease(needIncrease, handlerTimeout);
