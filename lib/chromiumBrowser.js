...
class ChromiumBrowser {
  constructor(debug = false, local = false) {
    ...
    // Production (local=false) keeps the container's Chromium; the Dockerfile also sets
    // CHROMIUM_EXECUTABLE_PATH to this same path, so the prod path is unchanged. Host local
    // runs (local=true, env unset) fall back to Playwright's bundled Chromium (undefined).
    this.localExecutivePath = process.env.CHROMIUM_EXECUTABLE_PATH
      || (local ? undefined : '/usr/bin/chromium-browser');
  }
