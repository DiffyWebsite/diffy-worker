...
class Api {
  ...
  async uploadScreenshots(snapshotName, results) {
    ...
    for (let i = 0; i < results.length; i += CHUNK_SIZE) {
      ...
      formData.append('snapshotName', snapshotName?.trim() || `Test Snapshot - ${new Date().toISOString()}`);

      chunk.forEach((item, index) => {
        const hasAll = item.uri && item.breakpoint && item.filename && fs.existsSync(item.filename);
        if (!hasAll) {
          console.warn(`Skipping index ${index} — incomplete data`);
          return;
        }
      ...
      if (!snapshotId) {
        if (response && typeof response === 'object' && 'id' in response) {
          ...
          console.error('Invalid or empty response from snapshot creation:', response);
          throw new Error('Snapshot creation failed — no valid ID returned');
        }
      }
    }
