...
class Api {
  ...
  async uploadScreenshots (snapshotName, results) {
    ...
    results.forEach((item, i) => {
      if (!item.filename) {
        console.error(`Item ${i} is missing a screenshot filename.`);
      }
      if (!item.htmlFilename) {
        console.error(`Item ${i} is missing an HTML filename.`);
      }
      if (!item.jsConsoleFilename) {
        console.error(`Item ${i} is missing a JS console filename.`);
      }
    ...
    this.logger.info(`Files: ${results.length} - Sending screensot to Diffy`)

    const screenshotId = await this._makePostRequest(url, formData, true, true, this.uploadScreenshotTimeout)
    this.logger.info(`Saved screenshot id: ${screenshotId}`)
    return screenshotId
  }
