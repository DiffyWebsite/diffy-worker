  ...
  groupLog(url, level, message, meta) {
    ...
    if (!this.logTimers[url]) {
      this.logTimers[url] = setTimeout(() => {
        this.flushLogs(url);
      }, 10000); // Send aggregated logs every 5 seconds
    }
  }

  flushLogs(url) {
    if (this.logGroups[url] && this.logGroups[url].length > 0) {
      // Aggregate all log messages for the URL
      const aggregatedMessage = this.logGroups[url]
          .map(log => `[${log.level.toUpperCase()}] ${log.timestamp} ${log.message} ${JSON.stringify(log.meta)}`)
          .join('\n');

      // Use console or an external integration to send the aggregated log
      console.log(`Aggregated Logs for URL: ${url}\n${aggregatedMessage}`);
