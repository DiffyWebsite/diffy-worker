    ...
    if (this.logGroups[url] && this.logGroups[url].length > 0) {
      const aggregatedLog = {
        ...
        logs: this.logGroups[url].map((log) => ({
          logger_severity: log.level.toUpperCase(),
      ...
      this.logGroups[url].forEach((log) => {
        ...
        } else if (log.level === "warn" && highestSeverity !== "error") {
          highestSeverity = "warn";
        }
      });

      // Use the appropriate logger based on the highest severity level
      switch (highestSeverity) {
        case "info":
          this.infoLogger.info(aggregatedLog);
          break;
        case "warn":
          this.warnLogger.warn(aggregatedLog);
          break;
        case "error":
          this.errorLogger.error(aggregatedLog);
          break;
      }

      // Clear the group after flushing
