...
class Logger {
  ...
  flushLogs(url) {
    if (this.logGroups[url] && this.logGroups[url].length > 0) {
      for (const log of this.logGroups[url]) {
        const aggregatedLog = {
          url,
        ...
        switch (log.level.toLowerCase()) {
          case 'warn':
            this.warnLogger.warn(JSON.stringify(aggregatedLog));
            break;
          case 'error':
            this.errorLogger.error(JSON.stringify(aggregatedLog));
            break;
          default:
            this.infoLogger.info(JSON.stringify(aggregatedLog));
        }
      }
