...
class Logger {
  ...
  flushLogs(url) {
    if (this.logGroups[url] && this.logGroups[url].length > 0) {
      ...
      const formattedLog = JSON.stringify(aggregatedLog, null, 2);

      switch (highestSeverity) {
        case "info":
          this.infoLogger.info(formattedLog);
          break;
        case "warn":
          this.warnLogger.warn(formattedLog);
          break;
        case "error":
          this.errorLogger.error(formattedLog);
          break;
      }
