...
class Logger {
    ...
    log(level, identificationKey, message, meta = {}) {
        if (typeof identificationKey === 'string') {
            this.logger.error({ level, message, identificationKey, ...meta });
        } else {
            meta = message;
            message = identificationKey;
            identificationKey = 'N/A';
            meta = meta || {};
            this.logger.log({ level, message, identificationKey, ...meta });
        }
    }
